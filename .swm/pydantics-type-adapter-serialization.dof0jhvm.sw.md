---
title: Pydantics Type Adapter Serialization
---
This document will cover the process of Pydantic's type adapter serialization, which includes:
...
The function `_init_core_attrs` is used to initialize core attributes of the schema. It sets the `_core_schema`, `_validator`, and `_serializer` attributes of the schema.
...
The function `_get_schema` is used to get the schema of the type. It generates and cleans the schema.
...
The function `collect_definitions` is used to collect definitions from the schema.
...
The function `_convert_schema` is used to convert the original schema to a tagged union schema.
...
The function `definitions_schema` is used to generate a JSON schema that matches a schema that defines a JSON object with definitions.
