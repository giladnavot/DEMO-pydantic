---
title: Data Serialization Process
---
This document will cover the process of data serialization in the Pydantic library, which includes:
 ...
 7. Handling nullable schemas
 8. Handling union schemas
 9. Generating inner schemas
10. Defining schemas

```mermaid
graph TD;
  serializer:::mainFlowStyle --> _init_core_attrs
...
The function `_init_core_attrs` is used to initialize core attributes of the schema. It tries to get the core schema, validator, and serializer from the type. If these attributes are not found, it creates them.
...
The function `_get_schema` is used to get the schema of a type. It uses the `GenerateSchema` class to generate and clean the schema.
...
The function `collect_definitions` is used to collect definitions from the schema. It adds the definitions to a dictionary and returns a definitions schema.
...
The function `_convert_schema` is used to convert the original schema to a tagged union schema. It handles the case where the original schema is not a union and converts it to a union schema.
...
The function `nullable_schema` is used to generate a JSON schema that matches a schema that allows null values. It generates an inner JSON schema and returns a schema that matches either the inner schema or null.
...
# Handling union schemas

The function `union_schema` is used to generate a JSON schema that matches a schema that allows values matching any of the given schemas. It generates a schema for each choice and returns a schema that matches any of the generated schemas.
...
# Generating inner schemas

The function `generate_inner` is used to generate a JSON schema based on the input schema. It handles different types of schemas and generates the appropriate JSON schema.
...
The function `definitions_schema` is used to generate a JSON schema that defines a JSON object with definitions. It generates an inner schema for each definition and returns a schema that matches the generated schemas.
